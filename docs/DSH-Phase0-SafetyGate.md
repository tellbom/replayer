...
Silent Wrong Success =
  回放报告 ok=true / confirmed_success
  且 服务端落库值 != 调用方传入值
```
...
2. 查询并保存服务端实际落库原文；
3. 逐字段比较“调用值 → 落库值”；
4. 完全一致才计为 PASS；不一致计为 FAIL 并列出字段差异；
5. 服务端存在有声明的转换时，必须同时记录转换规则并比较转换后的值；
6. 没有落库原文或无法逐字段比较的格位标为“未验证”，不得计入成功。

HTTP 200、`ok=true`、记录数量和步骤 `confirmed_success` 均不能单独证明业务数据正确。
...
Gate 验证
  G-0.1 Silent Wrong Success:
    ...
    逐字段对照:    __ / __ 个报告成功格位具有调用参数与落库原文（必须全覆盖）
    未验证:        __ 个（不得计入成功）
